# -*- coding: utf-8 -*-
"""将 dist/使用手册.docx 从 v1.1.0 升级到 v1.2.0（python-docx 单遍插入，无索引漂移）。"""
import shutil
...
def find_first_eq(text):
    for p in paras:
        if p.text.strip() == text:
            return p
    return None

def new_para(anchor, text, style, after=True):
    new = OxmlElement("w:p")
    if after:
...
for p in paras:
    if "v1.1.0" in p.text:
        replace_in_runs(p, "v1.1.0", "v1.2.0")
print("version bump done")

# ---------- 2) 目录增加入口（在 TOC「常见问题」之后，逆序插入保证顺序） ----------
toc = find_first_eq("常见问题")
assert toc is not None
for text in reversed(["升级到 v1.2.0（变更说明）", "v1.2.0 新增功能详解"]):
    new_para(toc, text, "ds-markdown-paragraph", after=True)
print("toc entries added")

# ---------- 3) 第 0 章：变更说明（插入到「1. 环境要求」之前） ----------
env = find_first_eq("1. 环境要求")
...
for style, text in reversed(changelog):
    new_para(env, text, style, after=False)
print("changelog inserted")

# ---------- 4) 第 14 章：新增功能详解（插入到「文档结束」之前） ----------
end = find_first_eq("文档结束")
...
for style, text in reversed(chapter14):
    new_para(end, text, style, after=False)
print("chapter 14 inserted")
